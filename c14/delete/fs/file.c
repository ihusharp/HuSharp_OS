...
// 从文件 file 中读取 count 字节写入 buf， 返回读出的字节数
// 若到了文件尾， 则返回 -1
int32_t file_read(struct file* file, void* buf, uint32_t count) {
    uint32_t size = count, size_left = count;
    
    // 判断从此处读取 count 是否会到 文件尾, 标识剩余 size
    if ((file->fd_pos + count) > (file->fd_inode->i_size)) {
        size = file->fd_inode->i_size - file->fd_pos;// 标识还剩的大小
        size_left = size;
        if(size == 0) {
            printk("file_read: exceed max file_size! read file failed!\n");
            return -1;
        }        
    }
    ...
    if(read_blocks_num == 0) {// 表明在同一扇区进行读取
        ...
        if(block_read_start_idx < 12) {//若是读取为 直接块
            ...
            indirect_block_table = file->fd_inode->i_sectors[12];
            // 将一级索引表直接读到 all_blocks 中
            ide_read(cur_part->my_disk, indirect_block_table, all_blocks+12, 1);
        }
    } else {//说明需要跨块， 下面要分三种情况处理
        // 1、 12 个直接块够用
        if(block_read_end_idx < 12) {// 读取的最后一个块 位于 直接块
            ...
            // 2、 除了需要读入 i_sectors 数组， 还需要读入 一级间接块
    ...
    }// 至此 all_blocks 包括 所有需要读入的地址
    uint32_t sec_idx, sec_lba, sec_off_bytes, sec_left_bytes, chunk_size;
    uint32_t bytes_read = 0;
    uint8_t* buf_dst = (uint8_t*) buf;// 保存变量
    while(bytes_read < size) {
        // 与写入大致相同
        sec_idx = file->fd_pos / BLOCK_SIZE;
        sec_lba = all_blocks[sec_idx];
        sec_off_bytes = file->fd_pos % BLOCK_SIZE;// 起始处
        sec_left_bytes = BLOCK_SIZE - sec_off_bytes;// 偏移处
        chunk_size = size_left < sec_left_bytes ? size_left : sec_left_bytes;

        memset(io_buf, 0, BLOCK_SIZE);
        ide_read(cur_part->my_disk, sec_lba, io_buf, 1);
        memcpy(buf_dst, io_buf + sec_off_bytes, chunk_size);// 读取 chunk_size 字节

        buf_dst += chunk_size;
        file->fd_pos += chunk_size;
        bytes_read += chunk_size;// 记录读取字节数
        size_left -= chunk_size;
    }
    sys_free(all_blocks);
    sys_free(io_buf);
