...
#define	DESC_G_4K    1  // 粒度
#define	DESC_D_32    1  // 有效地址 和 操作数大小 为 32 位
#define DESC_L	     0	// 64位代码标记，此处标记为0便可。
#define DESC_AVL     0	// cpu不用此位，暂置为0  
#define DESC_P	     1  // 段是否存在于内存中
#define DESC_DPL_0   0  
#define DESC_DPL_1   1
#define DESC_DPL_2   2
#define DESC_DPL_3   3
...
// 第 0 个不可用，第 1 个为代码段，第 2 个为数据段和栈，第 3 个为显存段
# define SELECTOR_K_CODE ((1 << 3) + (TI_GDT << 2) + RPL0) // 指向内核代码段的选择子
# define SELECTOR_K_DATA ((2 << 3) + (TI_GDT << 2) + RPL0)
# define SELECTOR_K_STACK SELECTOR_K_DATA
# define SELECTOR_K_GS ((3 << 3) + (TI_GDT << 2) + RPL0)

/* 第3个段描述符是显存,第4个是tss */
// 第 5、 6 个为用户所需的代码段 和 数据（栈）段
...
#define EFLAGS_IF_1	(1 << 9)	// if为 1 ,开中断
#define EFLAGS_IF_0	0		// if为 0 ,关中断
