...
int32_t sys_read(int32_t fd, void* buf, uint32_t count) {
    ...
    if(fd < 0 || fd == stdout_no || fd == stderr_no) {
        ...
        while(bytes_read < count) {
            // kbd_buf 是存储键盘输入的环形缓冲区
            *buffer = ioq_getchar(&kbd_buf);// extern 位于 keyboard.h 中
            bytes_read++;
            buffer++;
        }
        ret = (bytes_read == 0 ? -1 : (int32_t)bytes_read);// 成功则返回读取字节数
    } else {
        // 得到 fd 在文件表的下标
        uint32_t _fd = fd_local2global(fd);// 否则是读取文件
