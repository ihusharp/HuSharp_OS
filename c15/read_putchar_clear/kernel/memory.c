...
// 安排 1 页大小的 vaddr， 专门针对 fork 时 虚拟地址位图无须操作的情况
void* get_a_page_without_opvaddrbitmap(enum pool_flags pf, uint32_t vaddr) {
    struct pool* mem_pool = pf & PF_KERNEL ? &kernel_pool : &user_pool;
    lock_acquire(&mem_pool->lock);// 配锁
...
// 向物理池 回收 物理地址 pg_phy_addr
void pfree(uint32_t pg_phy_addr) {
    struct pool* mem_pool;
    uint32_t bit_idx = 0;
    if (pg_phy_addr >= user_pool.phy_addr_start) {//位于用户物理地址
        mem_pool = &user_pool;
...
/*******   内存释放函数  与 malloc_page 以及 其中三个步骤相匹配 ******************************/
// 去掉页表中虚拟地址vaddr的映射,只去掉vaddr对应的pte ,改变 pte 中的 P 位即可
static void page_table_pte_remove(uint32_t vaddr) {
    uint32_t* pte = pte_ptr(vaddr);
    *pte &= ~PG_P_1;//将 P 位 置为 1
    // 更新 tlb 快表， 通过 invlpg 更新单条虚拟地址条目
...
static void vaddr_remove(enum pool_flags pf, void* _vaddr, uint32_t pg_cnt) {
    ...
    if (pf == PF_KERNEL) {// 为内核
        bit_idx_start = (vaddr - kernel_addr.vaddr_start) / PG_SIZE;
        while (cnt < pg_cnt) {// 将每一个页位图置 0 
            bitmap_set(&kernel_addr.vaddr_bitmap, bit_idx_start + cnt++, 0);
        }
    } else {    // 为用户虚拟地址池
        struct task_struct* cur_thread = running_thread();
...
***************************************************************/
    ...
    /***********   mfree_page 的原理是三个动作的合成:   ***********
    1.通过 pfree 清空物理地址位图中的相应位，回收物理地址
    2.通过 page_table_pte_remove 去掉页表中虚拟地址 vaddr的映射,只去掉 vaddr对应的pte 上的 P 位
    3.通过 vaddr_remove 在虚拟地址池中释放以_vaddr起始的连续pg_cnt个虚拟页地址
***************************************************************/
    uint32_t pg_phy_addr;
    uint32_t vaddr = (int32_t)_vaddr, page_cnt = 0;
    ASSERT(pg_cnt >= 1 && vaddr % PG_SIZE == 0);
    pg_phy_addr = addr_v2p(vaddr);// 得到虚拟地址对应的物理地址

    /* 确保待释放的物理内存在低端1M+1k大小的页目录+1k大小的页表地址范围外 */
    ASSERT((pg_phy_addr % PG_SIZE) == 0 && pg_phy_addr >= 0x102000);

    // 判断 pg_phy_addr 属于内核还是用户内存池
...
void sys_free(void* ptr) {
    ...
    if(ptr != NULL) {
        ...
        // 先判断是线程还是进程
        if(running_thread()->pgdir == NULL) {//是线程
        ...
        lock_acquire(&mem_pool->lock);
        struct mem_block* b = ptr;// 内存块指针 b
        struct arena* a = block2arena(b); // 把mem_block转换成arena,获取元信息
        ASSERT(a->large == 0 || a->large == 1);

        if(a->desc == NULL && a->large == true) {//说明为 大于 1024 的内存
            mfree_page(PF, a, a->cnt);// 释放大内存只能用页框级别
        } else { // 表示待释放为 小内存
            list_append(&a->desc->free_list, &b->free_elem);// 将内存块回收到 free_list 中
        // 先进行 arena 的 free_list 判断，若满，那么进行整体释放
            if(++a->cnt == a->desc->blocks_per_arena) { // 此时说明已经满了
                uint32_t block_idx;
                for (block_idx = 0; block_idx < a->desc->blocks_per_arena; block_idx++) {
                    struct mem_block* b = arena2block(a, block_idx);// 进行转换
                    ASSERT(elem_find(&a->desc->free_list, &b->free_elem));
                    list_remove(&b->free_elem);
                }// 至此 全部删除完毕,需要对 a 进行一页的删除
                mfree_page(PF, a, 1);
