    ...
    * 数组name每次的值分别是"a","b","c" */
    ...
    while (name[0]) { // 若第一个字符就是结束符,结束循环
        ...
        if (search_dir_entry(cur_part, parent_dir, name, &dir_e)) {
            ...
            if (HS_FT_DIRECTORY == dir_e.f_type) { // 如果被打开的是目录
                parent_inode_no = parent_dir->inode->i_no;
                dir_close(parent_dir);
                parent_dir = dir_open(cur_part, dir_e.i_no); // 更新父目录
                searched_record->parent_dir = parent_dir;
                continue;
            } else if (HS_FT_REGULAR == dir_e.f_type) { // 若是普通文件
...
int32_t sys_open(const char* pathname, uint8_t flags)
{
    ...
    if (!found && !(flags & OP_CREAT)) {
        printk("in path %s, file %s is`t exist\n",
            searched_record.searched_path,
            (strrchr(searched_record.searched_path, '/') + 1));
        dir_close(searched_record.parent_dir);
        return -1;
    } else if (found && flags & OP_CREAT) { // 若要创建的文件已存在
...
int32_t sys_write(int32_t fd, const void* buf, uint32_t count)
{
    ...
    if (wr_file->fd_flag & OP_WONLY || wr_file->fd_flag & OP_RDWR) {
        ...
        console_put_str("sys_write: not allowed to write file without flag OP_RDWR or OP_WRONLY\n");
        return -1;
    }
}
...
struct dir* sys_opendir(const char* name)
{
    ...
    if (inode_no == -1) { // 如果找不到目录,提示不存在的路径
        ...
        if (searched_record.file_type == HS_FT_REGULAR) {
            printk("%s is regular file!\n", name);
        } else if (searched_record.file_type == HS_FT_DIRECTORY) {
